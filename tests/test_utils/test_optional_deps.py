...
class TestCheckOptionalDependency:
    ...
    @pytest.mark.parametrize("module_name,extra_name", [
        ("os", None),
        ("sys", "system"),
        ("json", "json"),
        ("math", None),
    ...
    @pytest.mark.parametrize("module_name,extra_name,expected_message", [
        (
...
class TestPandasMixin:
    ...
    def test_import_pandas_success(self):
        ...
        with patch('email_widget.utils.optional_deps.import_optional_dependency') as mock_import:
...
class TestIntegrationScenarios:
    ...
    @pytest.mark.parametrize("scenario,module_name,extra_name", [
        ("基础库", "os", None),
        ("子模块", "urllib.parse", "network"),
        ("复杂名称", "some_complex_library_name", "extra_features"),
    ])
    def test_various_module_scenarios(self, scenario: str, module_name: str, extra_name: str):
        """测试各种模块场景的参数化测试"""
        # 测试存在的模块（使用标准库）
...
class TestEdgeCases:
    ...
    def test_special_characters_in_module_name(self):
        """测试模块名包含特殊字符的情况"""
        special_names = ["module-with-dash", "module.with.dots", "module_with_underscores"]
